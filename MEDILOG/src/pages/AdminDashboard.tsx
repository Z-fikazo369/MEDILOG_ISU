...
type ViewType = "dashboard" | "patientRecords" | "accounts" | "backup";
...
const AdminDashboard: React.FC = () => {
  ...
  }, [patientCurrentPage, sortConfig, recordType]);
  ...
  const loadRecords = async (page: number, sort: SortConfig[]) => {
    try {
      setLoading(true);
      const response = await medicalAPI.getAllRecords(recordType, page, sort);
  ...
  const loadStudentAccounts = async (page: number) => {
    try {
      setLoadingAccounts(true);
      const response = await authAPI.getAllStudentAccounts(page);
                    ...
                    <table className="table table-striped table-hover align-middle">
                      ...
                      <tbody>
                        {studentAccounts.map((account, index) => (
                          <tr key={account._id}>
                            <td>
                              {(accountCurrentPage - 1) * ROWS_PER_PAGE +
                                index +
                                1}
                            </td>
                        ...
                        <tbody>
                          {filteredRecords.length === 0 ? (
                            ...
                            filteredRecords.map((record, index) => (
                              <tr key={record._id}>
                                <td>
                                  {(patientCurrentPage - 1) * ROWS_PER_PAGE +
                                    index +
                                    1}
                                </td>
